-- =====================================================
-- Миграция таблицы materials с сохранением данных
-- =====================================================
...
-- Шаг 5: Мигрируем данные из backup (если возможно)
DO $$
DECLARE
...
BEGIN
    ...
    -- Предполагаем что старая таблица имела колонки: id, code, name, category, unit (text), price, description, supplier
    FOR old_material_record IN SELECT * FROM public.materials_backup LOOP
        BEGIN
            -- Находим соответствующую единицу измерения по короткому имени
            DECLARE
