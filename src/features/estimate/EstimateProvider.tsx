...
type EstimateAction =
  ...
  | { type: 'APPLY_PRICE_CHANGE'; payload: { field: 'workPrice' | 'materialPriceWithVAT' | 'deliveryPrice'; percentage: number } }
...
function estimateReducer(state: EstimateState, action: EstimateAction): EstimateState {
  switch (action.type) {
    ...
    case 'UPDATE_ITEM': {
      ...
      const filteredItems = applyFilters(updatedItems, state.filters, state.sortConfig)
    ...
    case 'SET_FILTERS': {
      ...
      const filteredItems = applyFilters(state.items, newFilters, state.sortConfig)
    ...
    case 'SET_SORT_CONFIG': {
      const filteredItems = applyFilters(state.items, state.filters, action.payload)
    ...
    case 'APPLY_VOLUME_CHANGE': {
      const updatedItems = EstimateCalculator.applyVolumeChange(state.items, action.payload)
      const calculations = EstimateCalculator.calculateTotals(updatedItems)
      const analytics = EstimateCalculator.generateAnalytics(updatedItems)
      const filteredItems = applyFilters(updatedItems, state.filters, state.sortConfig)
...
export const EstimateProvider: React.FC<EstimateProviderProps> = ({ children }) => {
  ...
  useEffect(() => {
    if (state.items.length > 0 && typeof globalThis !== 'undefined' && globalThis.localStorage) {
      globalThis.localStorage.setItem('estimate_data', JSON.stringify(state.items))
      globalThis.localStorage.setItem('estimate_modifications', JSON.stringify(state.modifications))
    }
  }, [state.items, state.modifications])

  // Загрузка из localStorage при инициализации
  useEffect(() => {
    const savedItems = typeof globalThis !== 'undefined' && globalThis.localStorage ? globalThis.localStorage.getItem('estimate_data') : null
    const savedModifications = typeof globalThis !== 'undefined' && globalThis.localStorage ? globalThis.localStorage.getItem('estimate_modifications') : null
...
export const useEstimateData = () => {
  const { items, filteredItems, calculations, analytics } = useEstimate()
  return { items, filteredItems, calculations, analytics }
}
