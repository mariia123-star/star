...
export const usePortalLogger = () => {
  ...
  const logCreate = useCallback((
    ...
    logAction('create', table_name, {
      record_id,
      new_values,
      changes_summary: changes_summary || `Создана новая запись в таблице ${table_name}`,
    })
  }, [logAction])
  ...
  const logUpdate = useCallback((
    ...
    logAction('update', table_name, {
      ...
      old_values,
      new_values,
      changes_summary: changes_summary || `Обновлена запись в таблице ${table_name}`,
    })
  }, [logAction])
  ...
  const logDelete = useCallback((
    ...
    logAction('delete', table_name, {
      record_id,
      old_values,
      changes_summary: changes_summary || `Удалена запись из таблицы ${table_name}`,
    })
  }, [logAction])
  ...
  const logView = useCallback((
    ...
    logAction('view', table_name, {
      record_id,
      changes_summary: changes_summary || `Просмотр данных в таблице ${table_name}`,
    })
  }, [logAction])
  ...
  const logExport = useCallback((
    ...
    logAction('export', table_name, {
      ...
      changes_summary: changes_summary || `Экспорт данных из таблицы ${table_name}`,
    })
  }, [logAction])
  ...
  const logImport = useCallback((
    ...
    logAction('import', table_name, {
      ...
      changes_summary: changes_summary || `Импорт данных в таблицу ${table_name}`,
    })
  }, [logAction])
